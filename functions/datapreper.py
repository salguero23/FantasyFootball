...
def engineer_data():
    ...
    latest_season.to_csv(f'{path}\\Data\\latestSeason.csv',index=False)
...
def scale_data(trainingColumns):
    path = os.getcwd()

    # Import data to scale
    df = pd.read_csv(f'{path}\\Data\\mlData.csv')
    ...
    lastSeason = pd.read_csv(f'{path}\\Data\\latestSeason.csv')
    PLAYER = lastSeason.Player
    lastSeason = lastSeason[trainingColumns]

    lastSeason = scaler.transform(lastSeason)
    lastSeason = pd.DataFrame(columns=trainingColumns,index=PLAYER,data=lastSeason)
